...
if __name__ == "__main__":
    ...
    parser.add_argument("--tokenizer_path", help="Path to pre-trained tokenizer or Huggingface name. If none, it will be set to model_path", type=str, default="tokenizer/byte_bpe_tokenizer_101")
    ...
    parser.add_argument("--seeds_file", help="Path to file containing seed words", type=str, required=True)
    ...
    with open(args.seeds_file, 'r') as f:
        seed_words = [line.strip() for line in f.readlines()]

    generations = []

    for seed_word in tqdm(seed_words[0:20000]):
        for i in range(int(args.num_generate/args.batch_size)):
            # Set seed for reproducibility
            torch.manual_seed(args.seed_offset + i)
            with torch.no_grad():
                input_text = f"{seed_word}"
                ...
                g = model.generate(input_ids, do_sample=True, max_length=2*args.maxchars+3, pad_token_id=tokenizer.pad_token_id, bad_words_ids=[[tokenizer.bos_token_id, 5]], num_return_sequences=args.batch_size, num_beams=args.num_beams, top_p=args.top_p/100, top_k=args.top_k, temperature=args.temperature)
    ...
    # Store passwords
    with open(os.path.join(args.out_path, args.filename), 'a') as f:
